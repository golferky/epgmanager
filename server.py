...
def cleanup_recording_temp():
    temp = Path(LOCAL_RECORDING_PATH)
    if not temp.exists():
        return
    removed = 0
    for item in temp.iterdir():
        try:
            ...
            log.warning(f"STARTUP CLEANUP SKIPPED -> {item} | {e}")
    if removed:
        log.info(f"STARTUP CLEANUP -> removed {removed} leftover temp item(s)")
...
def db_connect():
    return ProxyConnection("http://192.168.1.176:5000/db/execute", timeout=30)


def db_connect_quick(timeout_seconds: float = 5.0):
    return ProxyConnection("http://192.168.1.176:5000/db/execute", timeout=max(timeout_seconds, 20.0))
    ...
    try:
        ...
        if proc.returncode == 0:
            if partial_path.exists() and partial_path.stat().st_size > 1_000_000:
                ...
                try:
                    shutil.rmtree(local_job_folder, ignore_errors=True)
                    log.info(f"LOCAL RECORDING CLEANUP -> {local_job_folder}")
                except Exception as e:
